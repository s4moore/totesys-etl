...
def integration_event():
  ...
  "response": 200,
  "pkl_files_written": {
    "sales_order": "2025-03-07/10:58:10.330000/sales_order.pkl" # needs to have a path to a file currently in real s3 bucket
  },
  "triggerLambda2": True
}
...
class TestLambdaHandlerTransformIntegration:
    ...
    def test_saves_pq_files_to_s3(self, integration_event):
        ...
        response = s3.list_objects_v2(Bucket="totes-11-processed-data").get("Contents")
...
class TestLambdaHandlerTransformMocking:
    def test_uploads_file_to_s3(self, mock_event, mock_aws_with_buckets_and_glue):
        ...
        tables_written = [file["Key"].split("/")[0] for file in response["Contents"]]
        assert "fact_sales_order" in tables_written

    def test_returns_dict_with_files_written(self, mock_event, mock_aws_with_buckets_and_glue):
        output = lambda_handler(mock_event, {})
        assert output["response"] == 200
        assert "/fact_sales_order/" in output["parquet_files_written"]['fact_sales_order']
